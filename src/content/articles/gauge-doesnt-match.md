...
browserTitle: "What to Do When Your Gauge Doesn't Match the Pattern"
...
Row gauge is harder to fix than stitch gauge. Changing needle size affects both, and not always proportionally. If your stitch gauge matches but your row gauge is off, the practical approach is to convert the pattern's row-based instructions to measurements. Calculate how many inches the designer intended between decreases (using the pattern's row gauge), then work to that measurement using yours. The [gauge measurement guide](/articles/how-to-measure-knitting-gauge/) covers the numbers you need before making that conversion.
...
If the fabric quality is off at correct gauge, the options are: try a different yarn closer to the original's fiber content, or accept that this yarn-pattern combination isn't going to work. Not every yarn suits every pattern, even when the math cooperates. The [yarn substitution guide](/articles/how-to-substitute-yarn/) covers the fiber-matching side of this problem.
...
One exception: gauge affects yardage. If your gauge is off on a blanket, the finished area and fabric density may change enough to affect how much yarn you need. Recalculate yardage with the [Yarn Estimator](/tools/yarn-estimator/) before buying.
