...
export function getArticlePath(article: CollectionEntry<'articles'>, lang: Lang = (article.data.lang ?? 'en')): string {
  ...
  if (isTranslatedArticle(key)) {
    const translatedPath = (articleTranslations[key] as Partial<Record<Lang, string>>)[lang];
...
export function getFinnishArticleRouteSlug(key: string, fallbackSlug = key): string {
  if (isTranslatedArticle(key)) {
...
export function getGermanArticleRouteSlug(key: string, fallbackSlug = key): string {
  if (isTranslatedArticle(key)) {
    const dePath = (articleTranslations[key] as Partial<Record<Lang, string>>).de;
