...
const VAULT_DIR = '/home/emma/Documents/ObsidianVault/Projects/Active/KnitTools/Website/SEO/Articles';
const CATEGORY_MAP_FILE = '/home/emma/Documents/ObsidianVault/Projects/Active/KnitTools/Website/artikkelijaot.md';
...
function buildCategoryMap() {
  ...
  for (const rawLine of lines) {
    ...
    const headerMatch = line.match(/^([A-Za-z& ]+?)(?:\s*\(\d+\))?$/);
    if (headerMatch) {
    ...
    const itemMatch = line.match(/^[-*]\s+(\d{2})\s+[—\-–]/);
...
function extractMetadata(source) {
  const meta = { title: null, description: null, primary: null, secondaries: [] };

  const commentBlocks = [...source.matchAll(/<!--([\s\S]*?)-->/g)].map((m) => m[1]);
...
function processFile(filename, categoryMap) {
  ...
  if (!m) return { skipped: true, reason: `filename does not match NN-slug.md (${filename})` };
  const number = m[1];
  const slug = m[2];

  if (slug === PROTECTED_SLUG) {
    return { skipped: true, slug, number, reason: 'protected article 04, already in place' };
  }

  const category = categoryMap[number];
  if (!category) {
    return { skipped: true, slug, number, reason: `no category mapping for article ${number}` };
  }
  ...
  if (missing.length) {
    return { skipped: true, slug, number, reason: `missing comments: ${missing.join(', ')}` };
  }

  const tags = [];
  const seen = new Set();
  for (const tag of [meta.primary, ...meta.secondaries].filter(Boolean)) {
...
function main() {
  ...
  const allFiles = readdirSync(VAULT_DIR)
    .filter((f) => f.endsWith('.md'))
    .sort();

  const targets = args.single
    ? allFiles.filter((f) => f.startsWith(`${args.single}-`))
    : allFiles;
  ...
  for (const filename of targets) {
    ...
    if (args.dryRun) {
      ...
      console.log('[body: ' + result.body.length + ' chars, first 200 chars below]');
  ...
  console.log('\n=== Summary ===');
  console.log(`Mode: ${args.dryRun ? 'dry-run' : 'write'}${args.single ? ` (single ${args.single})` : ''}`);
  ...
  if (Object.keys(perCategory).length) {
    ...
    for (const k of Object.keys(perCategory).sort()) {
