...
- `src/scripts/sizeChartControls.ts` luo myös mobiilin koonvalitsimet `.size-table`-taulukoille `data-mobile-size-label`-tekstistä. Mobiilissa näytetään yksi kokopalsta kerrallaan, mutta desktopissa ja semanttisessa HTML:ssä säilyy koko taulukko.
- `src/scripts/responsiveTableControls.ts` muuntaa `data-mobile-table="cards"` -puikkokokotaulukot kapeilla näytöillä label–value-korteiksi taulukon omista sarakeotsikoista. Ota ohjain käyttöön englannin sivulla `ResponsiveTableControls`-komponentilla ja lokalisoiduilla sivuilla `enableResponsiveTableControls`-propilla; älä kopioi mobiililabeleita kielikohtaiseen JavaScriptiin.
